...
public class FBscript : MonoBehaviour {
	...
	public void InviteV()
	{
		FacebookManager.Instance.InvitarAmigosV ();
		GetNoInvitados();

	}

	private void GetNoInvitados()
	{
		if (FacebookManager.Instance.NumeroInvite == -1)
		{
			StartCoroutine(WaitForInvite());
		}
		else
		{
			string mensaje = "Has invitado a " + FacebookManager.Instance.NumeroInvite+" Amigos !!";
			FacebookManager.Instance.MostrarDialogo( mensaje, true);
		}
	}

	IEnumerator WaitForInvite()
	{

		while (FacebookManager.Instance.NumeroInvite == -1)
		{
			yield return null;
		}
	...
	{
		FacebookManager.Instance.CompartirLogro ();
	}
